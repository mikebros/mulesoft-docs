...
Yes!  link:mailto:cloudhub-support@mulesoft.com[File a support case] to enable global deployments for your account. CloudHub applications can be deployed to: US (East), US (West), Europe (Ireland or Frankfurt),  and Asia-Pacific (Sydney or Tokyo). Administrators can  link:/anypoint-platform-administration/managing-cloudhub-specific-settings[set the default region] on the Organization tab in Account Settings, but that region can be adjusted here when the application is deployed, if necessary.
...
You can easily integrate your application deployed to CloudHub with MySQL or Oracle using Amazon Relational Database Service (RDS). Refer to the link:http://aws.amazon.com/rds/[Amazon RDS documentation] to learn how to set up a new database.
